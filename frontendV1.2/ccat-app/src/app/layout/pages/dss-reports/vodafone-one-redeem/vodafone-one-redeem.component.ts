...
@Component({
    selector: 'app-vodafone-one-redeem',
    templateUrl: './vodafone-one-redeem.component.html',
    styleUrls: ['./vodafone-one-redeem.component.scss'],
})
export class VodafoneOneRedeemComponent implements OnInit,OnDestroy {
    reportData;
    reportsHeaders;
    ...
    dateTime = new Date();
    getVodafoneOneRedeemReportPermission: boolean;
    classNameCon = '';
    isopened: boolean;
    isopenedNav: boolean;
    ...
    setDataOfTable(isFirstRequest: boolean, reportDataReq: ReportRequest) {
        this.reportsService.allVodafoneOneRedeem$(reportDataReq).subscribe((res) => {
            if(res.statusCode === 0){
    ...
    setPermissions() {
        let viewReportPermission: Map<number, string> = new Map().set(290, 'vOnered');
        this.featuresService.checkUserPermissions(viewReportPermission);
        this.getVodafoneOneRedeemReportPermission = this.featuresService.getPermissionValue(290);
    }
