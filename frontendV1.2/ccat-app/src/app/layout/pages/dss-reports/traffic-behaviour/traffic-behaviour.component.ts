...
export class TrafficBehaviourComponent implements OnInit, AfterViewChecked {
  ...
  constructor(private trafficBehaviorService: TrafficBehaviorService,
  ...
  getTraffic() {
    ...
    this.trafficBehaviorService.getTrafficBehavior$(trafficFromtDate, trafficToDate).pipe(
      take(1),
      map(res => res.payload),
  ...
  setPermissions() {
    let findSubscriberPermissions: Map<number, string> = new Map()
      .set(154, 'GET_TRAFFIC_BEHAVIOR_REPORT')
