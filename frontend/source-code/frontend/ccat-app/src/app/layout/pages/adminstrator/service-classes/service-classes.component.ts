...
export class ServiceClassesComponent implements OnInit {
  ...
  constructor(private serviceClasses: ServiceClassesService,
  ...
  ngOnInit(): void {
    ...
    if (this.permissions.getAllServiceClass) {
      this.serviceClasses.allServiceClasses$.pipe(
        take(1),
  ...
  filterTable() {
    if (this.filterSearch) {
      const filteredTable = this.classes.filter(classes => classes.name.toLocaleLowerCase().match(this.filterSearch.toLocaleLowerCase()))
  ...
  deleteClass(code) {
    this.serviceClasses.deleteClass$(code).pipe(take(1)).subscribe(
      {
        next: (res) => {
          if (res.statusCode === 0) {
  ...
  onImport(event) {
    ...
    fileFormData.append('fileExt',
      this.uploadedFiles[0].name.substring(this.uploadedFiles[0].name.lastIndexOf('.'), this.uploadedFiles[0].name.length));
