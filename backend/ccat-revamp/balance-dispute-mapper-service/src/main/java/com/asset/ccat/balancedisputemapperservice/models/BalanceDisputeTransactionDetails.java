...
public class BalanceDisputeTransactionDetails implements Serializable, Comparable<Object> {
  ...
  public BalanceDisputeTransactionDetails() {
  ...
  public void setRoundedVolume(String roundedVolume) {
    if (Objects.nonNull(this.subType)) {
      if (!this.subType.equals("Mobile Telephony")) {
        this.roundedVolume = roundedVolume;
      } else {
        if (Objects.nonNull(roundedVolume) && !roundedVolume.trim().isEmpty()) {
          this.roundedVolume = String.valueOf(
              Math.ceil(Double.parseDouble(roundedVolume) / 60));
        } else {
          this.roundedVolume = "";
        }
      }
  ...
  public int compareTo(Object obj) {
    BalanceDisputeTransactionDetails model = (BalanceDisputeTransactionDetails) obj;
    int output = -1;
