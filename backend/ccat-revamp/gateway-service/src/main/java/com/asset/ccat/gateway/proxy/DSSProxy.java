...
public class DSSProxy {
 ...
 @LogExecutionTime
    public DSSReportModel getETopUpTransactionsReport(DSSReportRequest request) throws GatewayException{
        CCATLogger.DEBUG_LOGGER.info("Starting getETopUpTransactionsReport - call ods service");
        DSSReportModel reportModel = null;
        BaseResponse<DSSReportModel> response = null;
        try {
            Mono<BaseResponse<DSSReportModel>> res = webClient.post()
                    .uri(properties.getOdsServiceUrls()
                            + Defines.ODS_SERVICE.CONTEXT_PATH
                            + Defines.ODS_SERVICE.DSS_REPORT
                            + Defines.ODS_SERVICE.ETOPUP_TRANSACTIONS
                            + Defines.WEB_ACTIONS.GET_ALL)
                    .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                    .body(BodyInserters.fromValue(request))
                    .retrieve()
            ...
            CCATLogger.DEBUG_LOGGER.info("Error while retrieving ETopUpTransactionsReport ");
            CCATLogger.ERROR_LOGGER.error("Error while retrieving ETopUpTransactionsReport", ex);
            throw new GatewayException(ErrorCodes.ERROR.INTERNAL_SERVICE_UNREACHABLE, "ods-service [" + properties.getOdsServiceUrls() + "]");
        }
        return reportModel;
    }
@LogExecutionTime
    public DSSReportModel getOutgoingViewReport(DSSReportRequest request) throws GatewayException{
        CCATLogger.DEBUG_LOGGER.info("Starting getOutgoingViewReport - call ods service");
        DSSReportModel reportModel = null;
        BaseResponse<DSSReportModel> response = null;
        try {
            Mono<BaseResponse<DSSReportModel>> res = webClient.post()
                    .uri(properties.getOdsServiceUrls()
                            + Defines.ODS_SERVICE.CONTEXT_PATH
                            + Defines.ODS_SERVICE.DSS_REPORT
                            + Defines.ODS_SERVICE.OUTGOING_VIEW
                            + Defines.WEB_ACTIONS.GET_ALL)
                    .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                    .body(BodyInserters.fromValue(request))
                    .retrieve()
            ...
            CCATLogger.DEBUG_LOGGER.info("Error while retrieving OutgoingViewReport ");
            CCATLogger.ERROR_LOGGER.error("Error while retrieving OutgoingViewReport", ex);
            throw new GatewayException(ErrorCodes.ERROR.INTERNAL_SERVICE_UNREACHABLE, "ods-service [" + properties.getOdsServiceUrls() + "]");
        }
        return reportModel;
    }}
