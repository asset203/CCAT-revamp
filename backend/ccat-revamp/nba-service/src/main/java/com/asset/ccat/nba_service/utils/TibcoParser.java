...
public class TibcoParser {
  ...
  public static List<GiftModel> parseAssignGiftResponse(GetAllTibcoGiftsResponse response) throws NBAException {
    ...
    try {
      ...
      if (customerMarketingProducts != null && !customerMarketingProducts.isEmpty()) {
      ...
      throw new NBAException(ErrorCodes.ERROR.TIBCO_PARSING_FAIL, " Can't parsing Get-all Tibco gifts response : " + ex.getMessage());
    }
    return gifts;
  }
}
