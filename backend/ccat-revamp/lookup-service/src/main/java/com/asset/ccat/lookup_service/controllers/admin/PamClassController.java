 ...
 */
@RestController
@RequestMapping(value = Defines.ContextPaths.LOOKUPS + Defines.ContextPaths.PAM_CLASS)
public class PamClassController {

    @Autowired
    LkPamClassService lkPamService;

    @PostMapping(value = Defines.WEB_ACTIONS.GET_ALL)
    public BaseResponse<List<LkPamModel>> getAllPamClasses(HttpServletRequest req, @RequestBody GetAllPamsRequest request) throws  LookupException {
        ThreadContext.put("requestId", request.getRequestId());
        ThreadContext.put("sessionId", request.getSessionId());
        List<LkPamModel> response = lkPamService.retrievePamClasses();
    ...
    public BaseResponse addPamClass(HttpServletRequest req, @RequestBody AddPamRequest request) throws  LookupException {
        ThreadContext.put("requestId", request.getRequestId());
        ThreadContext.put("sessionId", request.getSessionId());
        lkPamService.addPamClass(request.getPam().getId(), request.getPam().getDescription());
        return new BaseResponse<>(ErrorCodes.SUCCESS.SUCCESS,
    ...
    public BaseResponse updatePamClass(HttpServletRequest req, @RequestBody UpdatePamRequest request) throws  LookupException {
        ThreadContext.put("requestId", request.getRequestId());
        ThreadContext.put("sessionId", request.getSessionId());
        lkPamService.updatePamClass(request.getId(), request.getDescription());
        return new BaseResponse<>(ErrorCodes.SUCCESS.SUCCESS,
                "success", Defines.SEVERITY.CLEAR, null);
    }
    ...
    public BaseResponse deletePamClass(HttpServletRequest req, @RequestBody DeletePamRequest request) throws  LookupException {
        ThreadContext.put("requestId", request.getRequestId());
        ThreadContext.put("sessionId", request.getSessionId());
        lkPamService.deletePamClass(request.getId());
        return new BaseResponse<>(ErrorCodes.SUCCESS.SUCCESS,
                "success", Defines.SEVERITY.CLEAR, null);
    }
